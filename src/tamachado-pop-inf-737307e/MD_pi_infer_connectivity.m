...
function O = MD_pi_infer_connectivity(V)
% try
    tic
    %% choose a dataset
    ...
    %% run spike inference
    [spikes S] = MD_pi_spike_inference(traces,V);
...
%         S{validIndices(ii)} = [];
    ...
    %% readd invalid indices to make connectivity matrix numbers correspond
    %% to cell ids
    
    Phattemp = zeros(length(Phat.omega),length(Phat.omega));
    Phattemp = Phat.omega;
    invalidindices = find(indices ==0);
    for r=1:length(invalidindices)
        for g=1:length(indices)
            if g == invalidindices(r)
                Phattemp(g+1:length(Phattemp)+1,:)=Phattemp(g:end,:);
                Phattemp(g,:)=0;
                Phattemp(:,g+1:length(Phattemp+1))=Phattemp(:,g:end);
                Phattemp(:,g)=0;
    ...
    %% save out stuff
    for ii = 1:length(S), O.N{ii} = S{ii}.n; O.F{ii}=traces(ii,:);end; O.Phat = Phat;
%     O.F = traces; 
    O.V = V; O.indices = indices;
