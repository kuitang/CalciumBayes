...
%     if flip == true, fc = -1; else fc =  1; end;
    ...
    V.F{cc} = signal % ### MD Changed from fc*signal;  ###
    % preprocess the trace
    V.T    = length(V.F{cc});
%     if 1
        V.F{cc} = V.F{cc}-min(V.F{cc}); V.F{cc}=V.F{cc}/std(V.F{cc});
        V.F{cc} = V.F{cc}/max(V.F{cc}); % realmin doesn't exist V.F{cc}=V.F{cc}+realmin;
...
%     end
    ...
    if isempty(spikes{cc})
        ...
        T.lam = 0.5;  %% Added by Mor.
        
        %% MD Added for portion only foopsi:
        P.lam = 0.5;
        fprintf('\nfast-oopsi\n')
        n = zeros(V.T,1);
        for MDcount = 1:numel(V.posspiketimes(cc,:))
            if V.posspiketimes(cc,MDcount)<500
                tempF = V.F{cc}(1:1000);
                tempT = T;
                tempT.Ncells=1;
                tempT.traces = T.traces(cc,1:1000);
                tempT.T = 1000;
                [test.n test.P test.V]= fast_oopsi(tempF,tempT,P);
%                 test = run_oopsi(tempF,tempT,P);
                n(1:1000,1) = test.n;
%                 struct{cc}.V = V;
                
                
            else
                if V.posspiketimes(cc,MDcount)+500>V.T
                    tempF = V.F{cc}(V.posspiketimes(cc,MDcount)-499:end);
                    tempT = T;
                    tempT.Ncells=1;
                    tempT.T = length(tempF);
                    tempT.traces = T.traces(:,V.posspiketimes(cc,MDcount)-499:end);
                    [test.n test.P test.V]= fast_oopsi(tempF,tempT,P);
                    n(V.posspiketimes(cc,MDcount)-499:end)=test.n;                
                else
                    tempF = V.F{cc}(V.posspiketimes(cc,MDcount)-499:V.posspiketimes(cc,MDcount)+500);
                    tempT = T;
                    tempT.Ncells=1;
                    tempT.T = 1000;
                    tempT.traces = T.traces(:,V.posspiketimes(cc,MDcount)-499:V.posspiketimes(cc,MDcount)+500);
                    [test.n test.P test.V]= fast_oopsi(tempF,tempT,P);
                    n(V.posspiketimes(cc,MDcount)-499:V.posspiketimes(cc,MDcount)+500,1)=test.n;
                end
            end
        end
        struct{cc}.P = test.P;
...
% %         Removed by MD  replaced by the above.
%         if V.fast
